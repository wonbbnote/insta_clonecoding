...
#         filename = f'{user_info["username"]}-{create_date}'  # 파일이름
...
# def test_get2():
#     nickname_receive = request.args.get('follower_give')
...
#     user_id = request.args.get('user_id')
#     # url_for 하고 user_id='test' 라고 했던 부분의 그 user_id 를 통해서 test 라는 값을 가져오는 겁니다! 즉, test 라는 값이 할당되겠죠?
#     user_info2 = db.user.find_one({"id": user_id})
#     # test 라는 id 를 가진 유저를 찾고, 그 유저의 정보를 user_info 에 저장합니다.
...
def api_valid():
    ...
    try:
        ...
        # 여기에선 그 예로 닉네임을 보내주겠습니다.
        userinfo = db.users.find_one({'username': payload['id']}, {'_id': 0})
        return jsonify({'result': 'success', 'user_id': userinfo['username']}, user=user_info)
    except jwt.ExpiredSignatureError:
        # 위를 실행했는데 만료시간이 지났으면 에러가 납니다.
        return jsonify({'result': 'fail', 'msg': '로그인 시간이 만료되었습니다.'})
    except jwt.exceptions.DecodeError:
...
def post_create():
    ...
    if request.method == 'POST':
        ...
        post_list = list(db.posts.find({}))
        count = len(post_list) + 1
        # pfilename = f'{user_info3["id"]}-{create_date}'  # 파일이름
